...
public class UserWrapper
{
    ...
    public string AddressId => Guest?.AddressId ?? Customer?.AddressId ?? Employee?.AddressId ?? ApplicationUser?.AddressId ?? throw new InvalidOperationException("No user is set");
    public Address? Address => Guest?.Address ?? Customer?.Address ?? Employee?.Address ?? ApplicationUser?.Address ?? null; //throw new InvalidOperationException("No user is set");
